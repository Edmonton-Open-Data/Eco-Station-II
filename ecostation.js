    ...
    d3.json("https://cdn.rawgit.com/Edmonton-Open-Data/Edmonton-Eco-Stations/master/data/Eco_Station_Monthly_Users.json",
        function(error, data) {
            ...
            */
            ...
            //chart variables
            var chartHeightScale = 0.31;
            var margin           = {
                    top:    Math.round(barChart.height() * 0.02, 1),
                    right:  Math.round(barChart.width() * 0.02, 1),
                    bottom: Math.round(barChart.height() * 0.12, 1),
                    left:   Math.round(barChart.width() * 0.058, 1)
                };
            var pieXscale      = 1.55;
            var pieYscale      = 2.2;
            var pieRscale      = 0.45;
            var pieInnerRscale = pieRscale * 0.65;

            //---------------------dc_charts---------------------
            charts
                    .filter(function(chart) { return chart != pieChart; })
                    .forEach(function(chart, index) {
                        chart
            ...
            pieChart.dimension(yearDimension)
                    ...
                    .on("pretransition.legend", function(chart) { 
                        chart.selectAll(".dc-legend-item text")      
                            ...
                            .append("tspan")
                            .attr('x', Math.round(pieChart.width() * 0.25, 1))
            ...
            button.onAdd = function(map){
                ...
                this._div.innerHTML = "<p class='button-info'>Location of Eco Stations</p>";
                return this._div;
            };
            button.addTo(map);
